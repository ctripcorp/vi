...
public class VIServer {
    ...
    public static ServletHandler bind(Server server,String id,boolean useVIAuthentication){
        ...
        System.setProperty("vi.context.path", id);
        try {
            handler = new ServletHandler();
            server.setHandler(handler);

            handler.addServletWithMapping(VIApiServlet.class, "/vi/api/*");
            handler.addServletWithMapping(StaticContentServlet.class, "/vi/*");
